...
public class Web3Auth: NSObject {
    ...
    public init(_ params: W3AInitParams) async throws {
        ...
        do {
            ...
            if(fetchConfigResult) {
                do {
                    let loginDetailsDict = try await sessionManager.authorizeSession(origin: "*")
    ...
    public func enableMFA(_ loginParams: W3ALoginParams? = nil) async throws -> Bool{
        ...
        if !sessionId.isEmpty {
            ...
            self.sessionManager.setSessionId(sessionId: _sessionId)
    ...
    public func launchWalletServices(chainConfig: ChainConfig, path: String? = "wallet") async throws {
        ...
        if !sessionId.isEmpty {
            ...
            //open url in webview
            // TODO: keyWindow needs to be replaced with an alternative, if app with multiple scenes use it, it will spawn a window across all connected scenes
